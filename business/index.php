...
<nav class="navbar navbar-expand-lg d-flex align-items-center bg-dark-blue">
 ...
 <div class=" py-3 text-center">
  ...
  <?php
        $title = "Business Department"; // Set the default title
    ...
    <button class="btn btn-primary my-3" data-bs-toggle="modal" data-bs-target="#Modal">Add Items</button>
        ...
        <table class="table table-hover table-bordered table-striped">
            ...
            if (isset($_GET['search']) && !empty($_GET['search'])) {
                $search = $_GET['search'];
                $query = "SELECT * FROM `business_department` WHERE `inventory-list` LIKE '%$search%' ORDER BY `inventory-list` $order";
            } else {
                $query = "SELECT * FROM `business_department` ORDER BY `inventory-list` $order";
            }
