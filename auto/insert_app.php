...
if(isset($_POST['add_item'])) {
    ...
    if(empty($inventoryList) || empty($description) || empty($measure) || empty($quantity) || empty($price) || empty($totalPrice) || empty($examination)) {
        ...
        $query =  "INSERT INTO auto_department (`inventory-list`, `description`, measure, quantity, price, `total-price`, examination) VALUES ('$inventoryList', '$description', '$measure', '$quantity', '$price', '$totalPrice', '$examination')";
